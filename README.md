...
# DataFrame写入数据库
ds.to_db(df_to_write_in, table_name, how='append')
...
```python
from pykoala.etl import TimeKit
...
#### etl.Excel
```python
from pykoala.etl import Excel

excel = Excel(work_dir)
sheet_list = [
	[df1, 'sheet_name1'],
	[df2,'sheet_name2']
]
...
file_path = excel.to_excel(file_name, sheet_list, fmt=None)

# 其他可选参数 fillna='',font='微软雅黑',font_color='black',font_size=11,column_width=17
```
...
#### service.Email
```python
from pykoala.service import Email
...
#### service.Wechat
```python
from pykoala.service import Wechat
